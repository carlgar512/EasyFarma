...
async function crearPDF(tratamientoCompleto: TratamientoCompleto, outputPath: string, usuario: Usuario): Promise<void> {
  ...
  return new Promise((resolve) => {
    ...
    doc.fillColor("black").font("Helvetica").text(`${usuario.getNombreUsuario()} ${usuario.getApellidosUsuario()}`);

    doc.fillColor("#2DD55B").font("Helvetica-Bold").text("DNI: ", { continued: true });
    doc.fillColor("black").font("Helvetica").text(usuario.getDni());

    doc.fillColor("#2DD55B").font("Helvetica-Bold").text("Fecha de Nacimiento: ", { continued: true });
    doc.fillColor("black").font("Helvetica").text(usuario.getFechaNac()).moveDown(1.5);
...
export async function generarPdfCifrado(dni: string, tratamientoCompleto: TratamientoCompleto, usuario: Usuario): Promise<string> {
  ...
  await crearPDF(tratamientoCompleto, tempPath, usuario);
